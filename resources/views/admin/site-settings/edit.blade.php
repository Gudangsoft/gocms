...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Site Setting</h1>
            <p class="text-gray-600 mt-2">Update configuration setting for {{ $siteSetting->site->name }}</p>
        </div>
        <a href="{{ route('admin.site-settings.show', $siteSetting) }}" 
           class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md font-medium transition duration-200">
            <i class="fas fa-arrow-left mr-2"></i>Back to Setting
        </a>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form action="{{ route('admin.site-settings.update', $siteSetting) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="site_id" id="site_id" required
                            ...
                            @foreach($sites as $site)
                                <option value="{{ $site->id }}" {{ old('site_id', $siteSetting->site_id) == $site->id ? 'selected' : '' }}>
                                    {{ $site->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="label" 
                               id="label"
                               value="{{ old('label', $siteSetting->label) }}"
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="date" {{ old('type', $siteSetting->type) == 'date' ? 'selected' : '' }}>Date</option>
                            <option value="url" {{ old('type', $siteSetting->type) == 'url' ? 'selected' : '' }}>URL</option>
                            <option value="email" {{ old('type', $siteSetting->type) == 'email' ? 'selected' : '' }}>Email</option>
                        </select>
                        @error('type')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="value" 
                               id="value_text"
                               value="{{ old('value', $siteSetting->value) }}"
                               class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <textarea name="value" 
                                  ...
                                  rows="4"
                                  style="display: none;"
                                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('value', $siteSetting->value) }}</textarea>
                        ...
                        <div id="value_boolean" style="display: none;" class="flex items-center">
                            ...
                            <input type="checkbox" 
                                   ...
                                   id="value_checkbox"
                                   value="1"
                                   {{ old('value', $siteSetting->value) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $siteSetting->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="sort_order" 
                                   id="sort_order"
                                   value="{{ old('sort_order', $siteSetting->sort_order) }}"
                                   min="0"
                                   class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div class="flex items-start">
                                <div class="flex h-5 items-center">
                                    ...
                                    <input id="is_active" 
                                           ...
                                           type="checkbox" 
                                           value="1"
                                           {{ old('is_active', $siteSetting->is_active) ? 'checked' : '' }}
                                           class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                </div>
                        ...
                        <div class="border-t border-gray-200 pt-6">
                            <h4 class="text-sm font-medium text-gray-900 mb-3">Setting Information</h4>
                            <div class="space-y-2 text-sm text-gray-600">
                                <div><strong>Created:</strong> {{ $siteSetting->created_at->format('M j, Y') }}</div>
                                <div><strong>Updated:</strong> {{ $siteSetting->updated_at->format('M j, Y') }}</div>
                                <div><strong>Current Site:</strong> {{ $siteSetting->site->name }}</div>
                            </div>
                        </div>
                    </div>
                </div>
